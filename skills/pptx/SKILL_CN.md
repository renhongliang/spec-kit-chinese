...
用户可能要求您创建、编辑或分析 .pptx 文件的内容。.pptx 文件本质上是一个包含 XML 文件和其他资源的 ZIP 归档文件，您可以读取或编辑。对于不同的任务，您有不同的工具和工作流可用。
...
### 原始 XML 访问
您需要原始 XML 访问用于：注释、演讲者备注、幻灯片布局、动画、设计元素和复杂格式。对于这些功能中的任何一个，您需要解包演示文稿并读取其原始 XML 内容。

#### 解包文件
`python ooxml/scripts/unpack.py <office_file> <output_dir>`

**注意**：unpack.py 脚本位于相对于项目根的 `skills/pptx/ooxml/scripts/unpack.py`。如果此路径不存在脚本，使用 `find . -name "unpack.py"` 来定位它。
...
从头创建新 PowerPoint 演示文稿时，使用 **html2pptx** 工作流将 HTML 幻灯片转换为 PowerPoint，并具有精确定位。

### 设计原则

**关键**：在创建任何演示文稿之前，分析内容并选择适当的设计元素：
1. **考虑主题**：此演示文稿是关于什么的？它暗示什么语调、行业或情绪？
...
- **敢于冒险**：尝试意想不到的组合 - 医疗保健演示文稿不一定是绿色，金融不一定是海军蓝
...
8. **粉红色和紫色**：粉红色 (#F8275B)、珊瑚色 (#FF574A)、玫瑰色 (#FF737D)、紫色 (#3D2F68)
9. **酸橙和梅子**：酸橙 (#C5DE82)、梅子 (#7C3A5F)、珊瑚色 (#FD8C6E)、蓝灰色 (#98ACB5)
...
17. **海岸玫瑰**：旧玫瑰 (#AD7670)、海狸 (#B49886)、蛋壳色 (#F3ECDC)、灰灰色 (#BFD5BE)
...
### 布局提示
**创建带有图表或表格的幻灯片时：**
- **两列布局（首选）**：使用跨越全宽的标题，然后下方两列 - 一列中的文本/项目符号和另一列中的特色内容。这提供了更好的平衡，使图表/表格更易读。使用不等列宽的 flexbox（例如，40%/60% 分割）以优化每种内容类型的空间。
...
### 工作流
1. **必需 - 阅读整个文件**：从头到尾完整阅读 [`ooxml.md`](ooxml.md)（约 500 行）。**阅读此文件时永远不要设置任何范围限制。**在任何演示文稿编辑之前，阅读完整文件内容以了解 OOXML 结构和编辑工作流的详细指导。
...
3. **基于模板清单创建演示文稿大纲**：
   ...
   * **关键：将布局结构与实际内容匹配**：
     - 单列布局：用于统一叙述或单一主题
     - 两列布局：仅在您恰好有 2 个不同项目/概念时使用
     - 三列布局：仅在您恰好有 3 个不同项目/概念时使用
...
4. **使用 `rearrange.py` 复制、重新排序和删除幻灯片**：
   * 使用 `scripts/rearrange.py` 脚本创建具有所需顺序幻灯片的新演示文稿：
     ...
     ```
   * 脚本自动处理复制重复幻灯片、删除未使用的幻灯片和重新排序
   * 幻灯片索引是 0 基础的（第一张幻灯片是 0，第二张是 1，等等）
