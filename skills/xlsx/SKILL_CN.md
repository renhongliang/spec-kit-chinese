---
name: xlsx
description: "支持公式、格式、数据分析和可视化的综合电子表格创建、编辑和分析。当 Claude 需要处理电子表格（.xlsx、.xlsm、.csv、.tsv 等）时：(1) 创建带有公式和格式的新电子表格，(2) 读取或分析数据，(3) 在保留公式的同时修改现有电子表格，(4) 在电子表格中进行数据分析和可视化，或 (5) 重新计算公式"
...
- **百分比**：默认为 0.0% 格式（一位小数）
- **倍数**：格式化为估值倍数（EV/EBITDA、P/E）的 0.0x
...
#### 硬编码的文档要求
- 在单元格旁边注释或注释（如果在表格末尾）。格式："来源：[系统/文档]，[日期]，[具体引用]，[如果适用，URL]"
- 示例：
...
用户可能要求您创建、编辑或分析 .xlsx 文件的内容。对于不同的任务，您有不同的工具和工作流可用。
...
### 使用 openpyxl
- 单元格索引是 1 基础的（row=1, column=1 指的是单元格 A1）
