...
<!-- 示例：III. 测试优先（不可协商） -->
[原则_3_描述]
<!-- 示例：TDD 强制：编写测试 → 用户批准 → 测试失败 → 然后实施；严格执行红-绿-重构周期 -->

### [原则_4_名称]
<!-- 示例：IV. 集成测试 -->
[原则_4_描述]
<!-- 示例：需要集成测试的重点领域：新库合约测试、合约更改、服务间通信、共享模式 -->

### [原则_5_名称]
<!-- 示例：V. 可观测性，VI. 版本控制和破坏性更改，VII. 简单性 -->
[原则_5_描述]
...
[治理规则]
<!-- 示例：所有 PR/审查必须验证合规性；复杂性必须证明合理；使用 [GUIDANCE_FILE] 进行运行时开发指导 -->
