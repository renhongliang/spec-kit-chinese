...
6. **产品文档管理（增量开发支持）**：在规范创建后，管理产品文档以支持增量开发：

   a. **确定产品文档路径**：
      - 产品文档应位于：`/memory/product.md`（与 `constitution.md` 相同的位置）
      - 产品模板位于：`.specify/templates/product-template.md` 或 `templates/product-template.md`

   b. **检查产品文档是否存在**：
      - 如果 `/memory/product.md` 不存在：
        1. 从模板创建新的产品文档
        2. 初始化产品信息：
           - 从功能描述推断产品名称（如果无法推断，使用"产品"作为占位符）
           - 设置初始版本为 1.0.0
           - 设置创建日期为当前日期
           - 从功能描述中提取产品定位信息（如果可能）
        3. 创建第一个产品特性条目，关联到当前功能规范
      - 如果 `/memory/product.md` 已存在：
        1. 读取现有产品文档
        2. 解析当前版本号（从文档头部或版本历史中提取）
        3. 确定是否需要版本更新（基于功能的重要性和影响范围）

   c. **提取功能信息用于产品特性**：
      - 从规范中提取：
        - 功能名称（从规范标题或功能描述）
        - 用户故事列表（从"用户场景与测试"章节）
        - 功能优先级（从用户故事的优先级推断，或使用默认 P2）
        - 业务价值（从成功标准或功能描述推断）
   ...
   d. **更新产品文档**：
      ...
      - **更新最后更新日期**：
        - 更新文档头部的"最后更新"字段为当前日期

   e. **建立用户故事与产品特性的关联**：
      - 在规范的用户故事部分，添加产品特性引用：
        - 在每个用户故事后添加：`**关联产品特性**：F-XXX - [特性名称]`
      - 确保双向关联：
        - 产品文档中的特性引用用户故事
        - 规范中的用户故事引用产品特性
   ...
   **重要提示**：
   - 产品文档是全局核心信息，应谨慎更新
   - 版本号应遵循语义化版本控制（主版本.次版本.修订版本）
   - 特性ID应保持唯一且递增
   - 每次更新产品文档时，保留历史信息（在版本历史中记录）

7. **规范质量验证**：在编写初始规范后，根据质量标准进行验证：
...
8. 报告完成情况，包括：
   ...
   - 提醒：后续命令（如 `/speckit.plan`）将参考产品文档中的信息
